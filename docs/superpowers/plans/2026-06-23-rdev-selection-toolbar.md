...
**Architecture:** rdev 钩子线程只缓存坐标 / 发 channel;独立处理线程跑严格门控(命中检测 + 拖选阈值 + generation 失效),通过纯函数判定。工具条不再抢焦点,隐藏由全局点击检测驱动(**按下即关、抬起在新选区重弹**)。划词自动弹出受设置开关控制。
...
git commit -m "feat: AppState 增 toolbar_rect / autopopup 字段"
```
...
- 外部点击关工具条、抬起在新选区重弹(按下即关 / 抬起即弹)→ Task 8 Press-外记 press + Release 判定 ✓
- 非选字拖动不弹(require_text 兜底)→ Task 6 trigger_at + Task 8 schedule_capture ✓
- 阈值 + 延迟后取词 → Task 3 常量 + Task 8 schedule_capture ✓
- 不触发诸类 → classify_release(Task 3)+ Press 分流(Task 8)✓
