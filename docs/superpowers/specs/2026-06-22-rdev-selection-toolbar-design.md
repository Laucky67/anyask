...
| 可见态下再划选 | **按下即关 + 抬起即重弹(不压制抬起)** | 工具条开着时在它之外按下,先关旧条,但把这次按下当作新划选起点;抬起若构成有效拖选,就在新选区重弹——符合"选了什么就弹什么"。早期"压掉这次抬起"的方案会把这次划选吞掉、逼用户选两遍,故弃用;仅保留"没真正选到字不弹"兜底(挡拖滚动条/窗口等非选字拖动)。 |
...
```
                 ...
                 for msg in rx: 跑门控状态机 (§6)
                   - 可见态点外部 → 主线程 hide,本次按下转为候选拖选
                   - 有效拖选 → spawn(延迟后 trigger_at,空选不弹)
```
...
- 拆分 `trigger`:
  - `pub fn trigger_at(app, anchor_x, anchor_y, require_text: bool)`:原 `trigger` 主体去掉鼠标查询;取词 + 写 pending(锚点用传入)+ ensure_window + emit。`require_text=true` 时取词为空(trim 后)直接返回不弹——划词路径用,挡住非选字拖动;`false`(热键路径)维持原样(空选也弹)。
  - `pub fn trigger(app)`:热键入口,读 `mouse_hook::last_position()`(`None` 回退主屏左上)→ `trigger_at(.., false)`。**保持 `fn(&AppHandle)` 签名,`shortcuts.rs` 零改动。**
  - 划词路径用**抬起时的 (x,y)** 调 `trigger_at(.., true)`(锚点精确,避免延迟期间鼠标移动跑偏)。
...
核心模型:**按下即关、抬起即弹。** 工具条可见时,在它之外按下鼠标一律先关掉它,并把这次按下当作「可能的新划选起点」;抬起若构成有效拖选,就在新选区重新弹出。普通点击(位移不足)只关不弹。这样"选完一段紧接着划选下一段"能连续重弹,无需选两遍。

**收到 `Press{x,y}`:** 先 `INPUT_GEN += 1`(作废第 4 步排期的过期延迟任务),再按 `toolbar_rect` 分流:

| `toolbar_rect` | 点在矩形内? | 动作 |
|---|---|---|
| `Some`(可见) | 内 | 工具条按钮点击,交给按钮;`press=None` |
| `Some`(可见) | 外 | 主线程 `hide` → `press=Some(x,y,now)`(关旧条 + 开始候选拖选) |
| `None`(隐藏) | — | `press=Some(x,y,now)` |

> 「可见/外」与「隐藏」两行动作一致(都记 `press`),仅前者多一步 `hide`。不再压制抬起——这正是连续划选能重弹的关键。

**收到 `Release{x,y}`(按顺序):**
1. `let Some((px,py,t0)) = press.take() else continue` — 无记录的按下(如点在工具条内部)直接忽略
2. `if !autopopup_enabled { continue }` — 设置关了
3. `if toolbar_rect.is_some() { continue }` — 工具条仍可见(如上次延迟弹窗在本次拖动中途已弹),不重复触发
4. `dist = max(|x-px|,|y-py|)`、`dur = t0.elapsed()`;**`dist≥DRAG_DIST_PX && dur≥DRAG_MIN_MS`** 才算有效拖选 → 快照 `g = INPUT_GEN`,`spawn(sleep(DELAY_MS) → 二次校验 → trigger_at(x,y, require_text=true))`
   ...
   - `require_text=true`:取词为空则不弹(挡住可见态下拖滚动条/窗口等"非选字拖动";见 §5.3)。

"不触发"诸类:普通点击(dist 不足)、点工具条内部(press 置 None → 无记录)、设置关闭 / 工具条仍可见(第 2、3 步)、有效拖选但没选到字(第 4 步 `require_text` 兜底)。延迟期内的新输入由 `INPUT_GEN` + 二次校验作废,避免过期取词/弹窗。
...
- **Rust 纯函数单测**(镜像现有 `clamp_to_monitor` / `visible_toggle_action` 风格):
  ...
  - `classify_release(had_press, enabled, visible, dist, dur_ms) -> ReleaseAction { Trigger, Ignore }`(或等价签名)——覆盖不触发诸类 + 有效拖选。
...
- **手动验证**(rdev 是真 OS 钩子,必须手验):
  ...
  9. **工具条显示时直接划选另一段** → 旧条关闭、在新选区重新弹出(连续划选不必选两遍)
  10. 工具条显示时拖滚动条/窗口(非选字) → 不弹(`require_text` 兜底)
...
- **可见态非选字拖动**:工具条开着时拖滚动条/窗口也算"拖动",靠 `trigger_at(require_text=true)` 取词为空不弹兜底;若 `get-selected-text` 偶发返回旧剪贴板内容仍可能误弹一次——可接受,设置开关可彻底关掉。
...
src-tauri/src/state.rs                             +2 字段(toolbar_rect / autopopup)
