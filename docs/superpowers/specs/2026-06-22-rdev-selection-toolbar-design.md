...
pub selection_autopopup_enabled: AtomicBool,          // 设置开关, 默认 true
```
...
5. `dist = max(|x-px|,|y-py|)`、`dur = t0.elapsed()`;**`dist≥DRAG_DIST_PX && dur≥DRAG_MIN_MS`** → `spawn(sleep(DELAY_MS) 后 trigger_at(x,y))`

该表精确覆盖"不触发"四类:普通点击(dist 不足)、点工具条内部(press 置 None)、刚隐藏后的 mouse_up(suppress / press=None)、工具条可见(第 4 步)。
...
- Rust:`setup` 从 `read_settings` 初始化 `selection_autopopup_enabled` 原子;新增命令 `set_selection_auto_popup(app, enabled)` 写原子(在 `lib.rs` 的 `generate_handler!` 注册)。
- 前端切换时与 `updateSettings` 并行调用该命令。
